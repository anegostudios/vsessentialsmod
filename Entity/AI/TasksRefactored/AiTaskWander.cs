...
public class AiTaskWanderR : AiTaskBaseR
{
    ...
    public override bool ShouldExecute()
    {
        ...
        double distanceToSpawnSquared = entity.Pos.XYZ.SquareDistanceTo(spawnPosition.X, spawnPosition.Y, spawnPosition.Z);
        ...
        if (currentTimeMs - lastTimeInRangeMs > Config.TeleportToSpawnTimeout && entity.World.GetNearestEntity(entity.Pos.XYZ, Config.NoPlayersRange, Config.NoPlayersRange, target => target is EntityPlayer) == null)
        {
    ...
    public override bool ContinueExecute(float dt)
    {
        ...
        */
        ...
        if (entity.Controls.IsClimbing && entity.Properties.CanClimbAnywhere && entity.ClimbingIntoFace != null)
        {
            ...
            if (Math.Sign(facing.Normali.X) == Math.Sign(pathTraverser.CurrentTarget.X - entity.Pos.X))
            {
                pathTraverser.CurrentTarget.X = entity.Pos.X;
            }

            if (Math.Sign(facing.Normali.Y) == Math.Sign(pathTraverser.CurrentTarget.Y - entity.Pos.Y))
            {
                pathTraverser.CurrentTarget.Y = entity.Pos.Y;
            }

            if (Math.Sign(facing.Normali.Z) == Math.Sign(pathTraverser.CurrentTarget.Z - entity.Pos.Z))
            {
                pathTraverser.CurrentTarget.Z = entity.Pos.Z;
            }
        }
    ...
    {
        entity.Attributes.SetDouble("spawnX", entity.Pos.X);
        entity.Attributes.SetDouble("spawnY", entity.Pos.Y);
        entity.Attributes.SetDouble("spawnZ", entity.Pos.Z);
        spawnPosition.Set(entity.Pos.XYZ);
    }
    ...
    protected virtual bool LoadNextWanderTarget()
    {
        ...
        for (int check = 0; check < Config.MaxBlocksChecked; check++)
        {
            ...
            currentTarget.X = entity.Pos.X + dx;
            currentTarget.Y = entity.Pos.InternalY + dy;
            currentTarget.Z = entity.Pos.Z + dz;
            currentTarget.W = 1;
            ...
            switch (habitat)
            {
                ...
                case EnumHabitat.Land:
                    ...
                    else
                    {
                        ...
                        Vec3d startAhead = new Vec3d(entity.Pos.X, entity.Pos.Y, entity.Pos.Z).Ahead(1, 0, angleHor); // Otherwise they are forever stuck if they stand over the edge
