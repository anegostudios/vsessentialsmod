...
public class AiTaskLookAtEntityR : AiTaskBaseTargetableR
{
    ...
    public override bool ContinueExecute(float dt)
    {
        ...
        entity.Pos.Yaw += GameMath.Clamp(yawDistance, -currentTurnRadPerSec * dt * GlobalConstants.OverallSpeedMultiplier, currentTurnRadPerSec * dt * GlobalConstants.OverallSpeedMultiplier);
...
public sealed class AiTaskLookAtEntityConversable : AiTaskBaseR
{
    ...
    public override bool ContinueExecute(float dt)
    {
        ...
        targetVec.Set(
            (float)(target.Pos.X - entity.Pos.X),
            (float)(target.Pos.Y - entity.Pos.Y),
            (float)(target.Pos.Z - entity.Pos.Z)
        );

        float desiredYaw = (float)Math.Atan2(targetVec.X, targetVec.Z);

        float yawDistance = GameMath.AngleRadDistance(entity.Pos.Yaw, desiredYaw);
        entity.Pos.Yaw += GameMath.Clamp(yawDistance, -curTurnRadPerSec * dt, curTurnRadPerSec * dt);
        entity.Pos.Yaw %= GameMath.TWOPI;
