...
public abstract class AiTaskBaseTargetableR : AiTaskBaseR, IWorldIntersectionSupplier
{
    ...
    {
        if (entity.Pos.Dimension != target.Pos.Dimension) return false;
        ...
        if (rangeMultiplier != 1 && entity.Pos.DistanceTo(target.Pos) > range * rangeMultiplier)
        {
            return false;
        }
    ...
    protected virtual bool SearchForTarget()
    {
        ...
        targetEntity = partitionUtil.GetNearestEntity(entity.Pos.XYZ, seekingRange, entity => IsTargetableEntity(entity, seekingRange), Config.SearchType);
