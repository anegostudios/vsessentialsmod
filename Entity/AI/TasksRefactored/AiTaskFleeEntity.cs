...
public class AiTaskFleeEntityR : AiTaskBaseTargetableR
{
    ...
    public override bool ContinueExecute(float dt)
    {
        ...
        if (targetEntity != null && entity.Pos.SquareDistanceTo(targetEntity.Pos) > currentFleeingDistance * currentFleeingDistance)
        {
            return false;
        }
    ...
    protected override bool CheckEntityLightLevel()
    {
        ...
        if (Config.IgnoreDeepDayLight && entity.Pos.Y < world.SeaLevel + Config.DeepDayLightLevelOffset) return false;
    ...
    protected virtual bool TryInstaFlee()
    {
        ...
        if (targetEntity == null || entity.Pos.DistanceTo(targetEntity.Pos) > Config.SeekingRange)
        {
            float cosYaw = GameMath.Cos(entity.Pos.Yaw);
            float sinYaw = GameMath.Sin(entity.Pos.Yaw);
            double offset = 200;
            targetPos.Set(entity.Pos.X + sinYaw * offset, entity.Pos.Y, entity.Pos.Z + cosYaw * offset);
            targetYaw = entity.Pos.Yaw;
            targetEntity = null;
        }
        else
        {
            currentFleeingDistance = (float)entity.Pos.DistanceTo(targetEntity.Pos) + Config.FleeSeekRangeDifference;
    ...
    protected void UpdateTargetPosFleeMode(Vec3d targetPos, float yaw)
    {
        ...
        tmpVec1.Set(entity.Pos.X, entity.Pos.Y, entity.Pos.Z);
