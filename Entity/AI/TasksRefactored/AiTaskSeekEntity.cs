...
public class AiTaskSeekEntityR : AiTaskBaseTargetableR
{
    ...
    public override bool ShouldExecute()
    {
        ...
        if (ShouldRetaliate() && attackedByEntity != null)
        {
            ...
            targetPosition.SetWithDimension(attackedByEntity.Pos);
            AlarmHerd();
            return true;
        }
        ...
        targetPosition.SetWithDimension(targetEntity.Pos);
        if (entity.Pos.SquareDistanceTo(targetPosition) <= MinDistanceToTarget(Config.ExtraTargetDistance))
        {
            return false;
        }
    ...
    public override bool ContinueExecute(float dt)
    {
        ...
        {
            entity.Pos.Motion.X = jumpHorizontalVelocity.X;
            entity.Pos.Motion.Z = jumpHorizontalVelocity.Z;
        }
        ...
        if (attackPattern == EnumAttackPattern.DirectAttack)
        {
            ...
            pathTraverser.CurrentTarget.Y = targetEntity.Pos.InternalY;
    ...
    public override void OnEntityHurt(DamageSource source, float damage)
    {
        ...
        double distance = targetEntity.Pos.DistanceTo(entity.Pos);
    ...
    protected override bool SearchForTarget()
    {
        ...
        posBuffer.SetWithDimension(entity.Pos);
    ...
    {
        if (targetPosition.DistanceTo(entity.Pos.XYZ) < currentSeekingRange && !TryCircleTarget())
        {
    ...
    protected virtual bool TryCircleTarget()
    {
        ...
        float angle = (float)Math.Atan2(entity.Pos.X - targetPosition.X, entity.Pos.Z - targetPosition.Z);
    ...
    protected virtual double GetDistanceToTarget()
    {
        ...
        Cuboidd targetBox = targetEntity.SelectionBox.ToDouble().Translate(targetEntity.Pos.X, targetEntity.Pos.InternalY, targetEntity.Pos.Z);
        posBuffer.SetWithDimension(entity.Pos);
        posBuffer.Add(0, entity.SelectionBox.Y2 / 2, 0).Ahead(entity.SelectionBox.XSize / 2, 0, entity.Pos.Yaw);
    ...
    protected virtual void UpdatePath()
    {
        ...
        if (attackPattern == EnumAttackPattern.DirectAttack &&
            ...
            targetPosition.SquareDistanceTo(targetEntity.Pos.X, targetEntity.Pos.InternalY, targetEntity.Pos.Z) >= Config.MinDistanceToUpdatePath * Config.MinDistanceToUpdatePath)
        {
            targetPosition.Set(targetEntity.Pos.X + targetEntity.Pos.Motion.X * Config.MotionAnticipationFactor, targetEntity.Pos.InternalY, targetEntity.Pos.Z + targetEntity.Pos.Motion.Z * Config.MotionAnticipationFactor);
    ...
    protected virtual bool PerformJump()
    {
        ...
        if (distance >= Config.DistanceToTargetToJump[0] && distance <= Config.DistanceToTargetToJump[1] && !recentlyJumped && Config.MaxHeightDifferenceToJump >= entity.Pos.Y - targetEntity.Pos.Y)
        {
            Vec3d horizontalMotion = new(entity.Pos.Motion.X, 0, entity.Pos.Motion.Z);
            double horizontalSpeed = horizontalMotion.Length();

            double dx = (targetEntity.Pos.X - entity.Pos.X + targetEntity.Pos.Motion.X * Config.JumpMotionAnticipationFactor) * Config.JumpSpeedFactor * 0.033f * 0.5f;
            double dz = (targetEntity.Pos.Z - entity.Pos.Z + targetEntity.Pos.Motion.Z * Config.JumpMotionAnticipationFactor) * Config.JumpSpeedFactor * 0.033f * 0.5f;
            double dy = Config.JumpHeightFactor * GameMath.Max(Config.MaxVerticalJumpSpeed, (targetEntity.Pos.Y - entity.Pos.Y) * 0.033f);

            horizontalMotion.Set(dx, 0, dz).Normalize().Mul(horizontalSpeed);

            jumpHorizontalVelocity.Set(dx + entity.Pos.Motion.X, 0, dz + entity.Pos.Motion.Z);

            entity.Pos.Motion.X = horizontalMotion.X;
            entity.Pos.Motion.Z = horizontalMotion.Z;

            entity.Pos.Motion.Add(dx, dy, dz);
        ...
        {
            entity.Pos.Motion *= Config.AfterJumpSpeedReduction;
        }
