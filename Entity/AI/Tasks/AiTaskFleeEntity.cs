...
namespace Vintagestory.GameContent
{
    public class AiTaskFleeEntity : AiTaskBaseTargetable
    {
        ...
        public AiTaskFleeEntity(EntityAgent entity, JsonObject taskConfig, JsonObject aiConfig) : base(entity, taskConfig, aiConfig)
        {
            ...
            intelligentFlee = taskConfig["intelligentFlee"].AsBool(false);
            ...
            rndnum = entity.World.Rand.Next(2000);
        }
        ...
        protected override void SetDefaultValues()
        {
            ...
            ExecutionChance = 0.1;
            TargetOnlyInteractableEntities = false;
        }
        ...
        public override bool ShouldExecute()
        {
            ...
            if (minDayLight > 0)
            {
                ...
                if (ignoreDeepDayLight && entity.Pos.Y < world.SeaLevel - 2) return false;

                //Is the light level too weak to affect us? If so then return false because the light is too dim to make us flee.
                float sunlight = entity.World.BlockAccessor.GetLightLevel((int)entity.Pos.X, (int)entity.Pos.Y, (int)entity.Pos.Z, EnumLightLevelType.TimeOfDaySunLight) / (float)entity.World.SunBrightness;
                if (sunlight < minDayLight) return false;
            }

            int generation = GetOwnGeneration();
            float fearReductionFactor = (WhenInEmotionStates != null) ? 1 : Math.Max(0f, (tamingGenerations - generation) / tamingGenerations);
            ...
            if (targetEntity != null)
            {
                ...
                float yaw = (float)Math.Atan2(targetEntity.Pos.X - entity.Pos.X, targetEntity.Pos.Z - entity.Pos.Z);
        ...
        protected bool intelliFleePaused()
        {
            return intelligentFlee && entity.World.ElapsedMilliseconds - lastFailedPathSearchTotalMs < 9000 + rndnum;
        }

        private bool TryInstaFlee()
        {
            // Beyond visual range: Run in looking direction
            if (targetEntity == null || entity.Pos.DistanceTo(targetEntity.Pos) > seekingRange)
            {
                float cosYaw = GameMath.Cos(entity.Pos.Yaw);
                float sinYaw = GameMath.Sin(entity.Pos.Yaw);
                double offset = 200;
                targetPos = new Vec3d(entity.Pos.X + sinYaw * offset, entity.Pos.Y, entity.Pos.Z + cosYaw * offset);
                targetYaw = entity.Pos.Yaw;
                targetEntity = null;
            }
            else
            {
                nowFleeingDistance = (float)entity.Pos.DistanceTo(targetEntity.Pos) + 15;
                if (entity.ToleratesDamageFrom(targetEntity)) nowFleeingDistance /= 2.5f;
                updateTargetPosFleeMode(targetPos, entity.Pos.Yaw);
            }
        ...
        public override void StartExecute()
        {
            ...
            if (intelligentFlee)
            {
                ...
                pathTraverser.Flee_Async(targetEntity.Pos.XYZ, fleeingDistance, moveSpeed, OnGoalReached, OnStuck, OnINoPath, searchDepth, creatureType);
            }
            else
            {
        ...
        private void OnINoPath()
        {
            lastFailedPathSearchTotalMs = entity.World.ElapsedMilliseconds;
            entity.Attributes.SetLong("lastFailedPathSearchTotalMs", lastFailedPathSearchTotalMs);
            stuck = true;
        }

        public override bool CanContinueExecute()
        {
            if (intelligentFlee && pathTraverser.Ready)
            {
                entity.Attributes.SetLong("lastGoodPathSearchTotalMs", entity.World.ElapsedMilliseconds);
            }

            return !intelligentFlee || pathTraverser.Ready;
        }
        ...
        public override bool ContinueExecute(float dt)
        {
            if (!intelligentFlee)
            {
                ...
                {
                    float yaw = targetEntity == null ? -targetYaw : (float)Math.Atan2(targetEntity.Pos.X - entity.Pos.X, targetEntity.Pos.Z - entity.Pos.Z);
            ...
            if (targetEntity != null && entity.Pos.SquareDistanceTo(targetEntity.Pos) > nowFleeingDistance * nowFleeingDistance)
            {
                return false;
            }
            ...
            {
                float sunlight = entity.World.BlockAccessor.GetLightLevel((int)entity.Pos.X, (int)entity.Pos.Y, (int)entity.Pos.Z, EnumLightLevelType.TimeOfDaySunLight) / (float)entity.World.SunBrightness;
                if ((ignoreDeepDayLight && entity.Pos.Y < world.SeaLevel - 2) || sunlight < minDayLight)
                {
