...
namespace Vintagestory.GameContent
{
    ...
    public class AiTaskSeekEntity : AiTaskBaseTargetable
    {
        ...
        public AiTaskSeekEntity(EntityAgent entity, JsonObject taskConfig, JsonObject aiConfig) : base(entity, taskConfig, aiConfig)
        {
            ...
            if (!Enum.TryParse(taskConfig["searchType"].AsString(), out searchType))
            {
                searchType = EnumEntitySearchType.Creatures;
            }
        ...
        public override bool ShouldExecute()
        {
            ...
            if (ShouldRetaliateForRange(NowSeekRange))
            {
            ...
            else
            {
                ...
                if (OnIsSuitableTarget?.Invoke(targetEntity) == false) return false;
                ...
                return entity.Pos.SquareDistanceTo(targetPos) > MinDistanceToTarget();
            }
        }

        protected virtual bool GetIsTamed()
        {
            return GetOwnGeneration() >= tamingGenerations;
        }
        ...
        private bool noTagMatchAll(Entity e, bool fullyTamed)
        {
            if (fullyTamed && (isNonAttackingPlayer(e) || entity.ToleratesDamageFrom(attackedByEntity))) return false;
        ...
        protected float getSeekRange()
        {
            ...
            if (ShouldRetaliateForRange(NowSeekRange * 1.5f))
            {
                nowRange *= 1.5f;
            }
        ...
        {
            if (targetPos.DistanceTo(entity.Pos.XYZ) < NowSeekRange)
            {
                if (!TryCircleTarget())
                {
        ...
        private bool TryCircleTarget()
        {
            ...
            float angle = (float)Math.Atan2(entity.Pos.X - targetPos.X, entity.Pos.Z - targetPos.Z);
        ...
        public override bool ContinueExecute(float dt)
        {
            ...
            if (attackPattern == EnumAttackPattern.DirectAttack && lastPathUpdateSeconds >= 0.75f && targetPos.SquareDistanceTo(targetEntity.Pos.X, targetEntity.Pos.InternalY, targetEntity.Pos.Z) >= 3 * 3)
            {
                targetPos.Set(targetEntity.Pos.X + targetEntity.Pos.Motion.X * 10, targetEntity.Pos.InternalY, targetEntity.Pos.Z + targetEntity.Pos.Motion.Z * 10);
            ...
            if (!inCreativeMode && leapAtTarget && rand.NextDouble() < leapChance)
            {
                ...
                if (distance > 0.5 && distance < 4 && !recentlyJumped && targetEntity.Pos.Y + 0.1 >= entity.Pos.Y)
                {
                    double dx = (targetEntity.Pos.X + targetEntity.Pos.Motion.X * 80 - entity.Pos.X) / 30;
                    double dz = (targetEntity.Pos.Z + targetEntity.Pos.Motion.Z * 80 - entity.Pos.Z) / 30;
                    entity.Pos.Motion.Add(
                        dx,
                        leapHeightMul * GameMath.Max(0.13, (targetEntity.Pos.Y - entity.Pos.Y) / 30),
                        dz
                    );
        ...
        public override void OnEntityHurt(DamageSource source, float damage)
        {
            ...
            if (targetEntity == source.GetCauseEntity() || !active)
            {
                ...
                float dist = source.GetCauseEntity() == null ? 0 : (float)source.GetCauseEntity().Pos.DistanceTo(entity.Pos);
