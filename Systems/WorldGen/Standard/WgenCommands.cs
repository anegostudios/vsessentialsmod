...
namespace Vintagestory.ServerMods
{
    ...
    public class WgenCommands : ModSystem
    {
        ...
        public override void StartServerSide(ICoreServerAPI api)
        {
            ...
            genStructures = api.ModLoader.GetModSystem<GenStructures>();
        ...
        private void InitWorldGen()
        {
            ...
            _scfg = genStructures.scfg;
            _vcfg = genStructures.vcfg;
            ...
            api.ChatCommands.GetOrCreate("wgen")
                .BeginSubCommand("structures")
                    .BeginSubCommand("spawn")
                        .RequiresPlayer()
                        .WithDescription("Spawn a structure from structure.json like during worldgen. Target position will be the selected block or your position. Use /wgen structures list <num> to get the correct index.")
                    ...
                    .BeginSubCommand("list")
                        .WithDescription("List structures with their indices for the /wgen structure spawn command")
        ...
        private void CreateCommands(){
            ...
            api.ChatCommands.GetOrCreate("wgen")
                ...
                .BeginSubCommand("regenk")
                    ...
                    .WithAdditionalInformation("If run from the server console it will use the world center as a reference point.")
                ...
                .EndSubCommand()
                .BeginSubCommand("regenrange")
                    .WithDescription("Regenerate a range of chunks. Start and end positions are in chunk coordinates. See CTRL + F3")
                    .WithArgs(parsers.Int("x_start"),parsers.Int("z_start"), parsers.Int("x_end"),parsers.Int("z_end"))
                    .HandleWith(OnCmdRegenrange)
                .EndSubCommand()
        ...
        private TextCommandResult handleToggleImpresWgen(TextCommandCallingArgs args)
        {
            ...
            {
                return TextCommandResult.Success("Meta block replacing and Item resolving for worldgen currently " + (GlobalConfig.ReplaceMetaBlocks ? "on" : "off"));
            }

            var doReplace = (bool)args[0];
            GlobalConfig.ReplaceMetaBlocks = doReplace;
        ...
        private TextCommandResult OnCmdRegenk(TextCommandCallingArgs args)
        {
            ...
            var aroundPlayer = args.Caller.Player is IServerPlayer;

            GetCoordsFromRange(range, args.Caller.Player, out var coords, out var regCoords);
            return RegenChunks(args.Caller, coords, regCoords, landform, aroundPlayer);
        }
        ...
        private TextCommandResult OnCmdRegen(TextCommandCallingArgs args)
        {
            ...
            GetCoordsFromRange(range, args.Caller.Player, out var coords, out var regCoords);
            return RegenChunks(args.Caller, coords, regCoords, landform, aroundPlayer, true);
        }
        ...
        private TextCommandResult OnCmdRegenc(TextCommandCallingArgs args)
        {
            ...
            GetCoordsFromRange(range,null, out var coords, out var regCoords);
            return RegenChunks(args.Caller, coords, regCoords, landform, false, true);
        }

        private TextCommandResult OnCmdPregen(TextCommandCallingArgs args)
        {
            var range = (int)args[0];
            return PregenerateChunks(args.Caller, range);
        }
        ...
        private TextCommandResult OnCmdDelrock(TextCommandCallingArgs args)
        {
            var range = (int)args[0];
            var aroundPlayer = args.Caller.Player is IServerPlayer;
            DelRock(args.Caller, range, aroundPlayer);
            return TextCommandResult.Success();
        }
        ...
        private TextCommandResult OnCmdDel(TextCommandCallingArgs args)
        {
            ...
            GetCoordsFromRange(range, args.Caller.Player, out var coords, out var regCoords);
            return Regen(args.Caller, true, landform, coords, regCoords, aroundPlayer);
        }
        ...
        private TextCommandResult OnCmdDelr(TextCommandCallingArgs args)
        {
            ...
            var aroundPlayer = args.Caller.Player is IServerPlayer;
            GetCoordsFromRange(range, args.Caller.Player, out var coords, out var regCoords);
            return Regen(args.Caller, true, null, coords, regCoords, aroundPlayer, true);
        }

        private void GetCoordsFromRange(int range, IPlayer aroundPlayer, out List<Vec2i> coords ,out HashSet<Vec2i> regCoords)
        {
            coords = new List<Vec2i>();
            regCoords = new HashSet<Vec2i>();
            ...
            {
                chunkMidX = (int)aroundPlayer.Entity.Pos.X / GlobalConstants.ChunkSize;
                chunkMidZ = (int)aroundPlayer.Entity.Pos.Z / GlobalConstants.ChunkSize;
            }
        ...
        private TextCommandResult OnCmdRegenrange(TextCommandCallingArgs args)
        {
            var xs = (int)args[0];
            var zs = (int)args[1];
            var xe = (int)args[2];
            ...
            var regCoords = new HashSet<Vec2i>();
            ...
            for (int x = xs; x <= xe; x++)
            {
                for (int z = zs; z <= ze; z++)
                {
                    ...
                    regCoords.Add(new Vec2i(x / regionChunkSize, z / regionChunkSize));
                }
            }
            return RegenChunks(args.Caller, coords, regCoords);
        }
        ...
        private void DelRock(Caller caller, int rad, bool aroundPlayer = false)
        {
            ...
            var notification = "Deleting rock, this may take a while...";
            if (caller.Player is IServerPlayer sp)
            {
                sp.SendMessage(caller.FromChatGroupId, notification, EnumChatType.Notification);
            }
            else
            {
                api.Logger.Notification(notification);
            }
            int chunkMidX = api.WorldManager.MapSizeX / GlobalConstants.ChunkSize / 2;
            int chunkMidZ = api.WorldManager.MapSizeZ / GlobalConstants.ChunkSize / 2;

            if (aroundPlayer && player != null)
            {
            ...
            foreach (Vec2i coord in coords)
            {
                for (int cy = 0; cy < api.WorldManager.MapSizeY / GlobalConstants.ChunkSize; cy++)
                {
                    ...
                    for (int i = 0; i < chunk.Data.Length; i++)
                    {
                        ...
                        if (block.BlockMaterial == EnumBlockMaterial.Stone || block.BlockMaterial == EnumBlockMaterial.Water || block.BlockMaterial == EnumBlockMaterial.Soil)
                        {
                            chunk.Data[i] = 0;
                        }
                    }
        ...
        private TextCommandResult PregenerateChunks(Caller caller, int range)
        {
            int chunkMidX = api.WorldManager.MapSizeX / GlobalConstants.ChunkSize / 2;
            int chunkMidZ = api.WorldManager.MapSizeX / GlobalConstants.ChunkSize / 2;

            if (caller.Player is IServerPlayer player)
            {
                chunkMidX = (int)player.Entity.Pos.X / GlobalConstants.ChunkSize;
                chunkMidZ = (int)player.Entity.Pos.Z / GlobalConstants.ChunkSize;
            }
        ...
        private TextCommandResult RegenChunks(Caller caller, List<Vec2i> coords, HashSet<Vec2i> regCoords,string landform = null, bool aroundPlayer = false, bool deleteRegion = false)
        {
            TextCommandResult msg;

            var notification = "Waiting for chunk thread to pause...";
            if (caller.Player is IServerPlayer sp)
            {
                sp.SendMessage(caller.FromChatGroupId, notification, EnumChatType.Notification);
            }
            ...
            {
            	// api.Assets.Reload(new AssetLocation("worldgen/")); - this does not seem to properly reload the files?
                ...
                msg = Regen(caller, false, landform, coords, regCoords, aroundPlayer, deleteRegion);
            } else
            {
        ...
        TextCommandResult Regen(Caller caller, bool onlydelete, string landforms, List<Vec2i> coords, HashSet<Vec2i> regCoords, bool aroundPlayer = false, bool deleteRegion = false)
        {
            int chunkMidX = api.WorldManager.MapSizeX / GlobalConstants.ChunkSize / 2;
            int chunkMidZ = api.WorldManager.MapSizeZ / GlobalConstants.ChunkSize / 2;
            var player = caller.Player as IServerPlayer;
            ...
            var modSys = api.ModLoader.GetModSystem<ModSystemWorldGenStructures>();
            int regionChunkSize = api.WorldManager.RegionSize / GlobalConstants.ChunkSize;
            ...
            if (deleteRegion && !onlydelete)
            {
                ...
                foreach (Vec2i coord in coords)
                {
                    ...
                    if (mapRegion?.GeneratedStructures.Count > 0)
                    {
                        ...
                        modSys.TriggerStructuresDeleted(structures);
                ...
                foreach (Vec2i coord in coords)
                {
                    api.WorldManager.LoadChunkColumnPriority(coord.X, coord.Y, new ChunkLoadOptions()
                    {
                        OnLoaded = () => {
                            ...
                            {
                                modSys.TriggerRegenFinalized(chunkMidX, chunkMidZ);
                                sent = true;

                                api.Event.PushEvent("wgenregendone");
                                ...
                                {
                                    api.Logger.Notification("Regen complete");
                                }
                                ...
                                foreach (var regCoord in regCoords)
                                {
                                    api.WorldManager.BroadcastMapRegion(regCoord.X, regCoord.Y);
                                }
                            }
                        },
                        ChunkGenParams = tree
            ...
            if (onlydelete)
            {
                return TextCommandResult.Success("Deleted " + count + " chunk columns" + (deleteRegion ? " and regions" : ""));
            }

            return TextCommandResult.Success("Reloaded landforms and regenerating " + count + " chunk columns" + (deleteRegion ? " and regions" : ""));
        }
        ...
        private TextCommandResult OnStructuresSpawn(TextCommandCallingArgs args)
        {
            ...
            var pos = args.Caller.Player.CurrentBlockSelection?.Position.AddCopy(0, (float)struc.OffsetY, 0) ?? args.Caller.Pos.AsBlockPos.AddCopy(0, (float)struc.OffsetY, 0);
