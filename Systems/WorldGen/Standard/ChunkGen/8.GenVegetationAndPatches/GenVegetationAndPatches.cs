...
{
    public interface IBlockPatchModifier
    {
        bool PreventPlacementAt(int x, int z, int category);

        /// <summary>
        /// Called at the beginning of a chunk generation pass. If there is nothing preventing placement in a large radius, then we can generate more efficiently
        /// Return true with this method if that is the case. Rule of thumb: Test within a 3x3 chunk square of given coordinate
        /// </summary>
        /// <param name="chunkX"></param>
        /// <param name="chunkZ"></param>
        /// <returns></returns>
        bool PreventPlacementBroadlyAt(int chunkX, int chunkZ);

        BlockPatchConfig GetPatchProviderAt(int chunkX, int chunkZ, ref EnumHandling handling);
    }
    ...
    public class GenVegetationAndPatches : ModStdWorldGen
    {
        ...
        protected float forestMod;
        protected float shrubMod = 0f;
        ...
        public void RegisterPatchModifier(IBlockPatchModifier modifier)
        {
            patchMod = patchMod.Append(modifier);
        }
        ...
        public void initWorldGen()
        {
            ...
            var blockpatchesfiles = sapi.Assets.GetMany<BlockPatch[]>(sapi.World.Logger, "worldgen/blockpatches/").OrderBy(b=>b.Key.ToString());
        ...
        private bool canGenerateAt(int x, int z, int patchesHashCode)
        {
            foreach (var provider in patchMod)
            {
                if (provider.PreventPlacementAt(x, z, patchesHashCode)) return false;
            }

            return true;
        }

        private BlockPatch[] GetNonTreePatches(int chunkX, int chunkZ)
        {
            BlockPatch[] bpcPatchesNonTree = null;
            EnumHandling handling = EnumHandling.PassThrough;
            foreach (var provider in patchMod)
            {
                var herehandling = EnumHandling.PassThrough;
                var pbpc = provider.GetPatchProviderAt(chunkX, chunkZ, ref herehandling);
                if (herehandling == EnumHandling.PassThrough) continue;

                handling = herehandling;
                if (pbpc == null) continue;
                bpcPatchesNonTree = bpcPatchesNonTree == null ? pbpc.PatchesNonTree : bpcPatchesNonTree.Append(bpc.PatchesNonTree);
                if (herehandling == EnumHandling.PreventSubsequent) break;
            }
