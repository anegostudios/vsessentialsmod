...
namespace Vintagestory.ServerMods
{
    public class ForestFloorSystem
    {
        ...
        {
            forestBlocks = ForestFloorHelper.InitialiseForestBlocks(worldAccessor);
        ...
        internal void CreateForestFloor(IBlockAccessor blockAccessor, TreeGenConfig config, BlockPos pos, IRandom rnd, int treesInChunkGenerated)
        {
            ...
            BlockPos currentPos = new BlockPos(API.Config.Dimensions.NormalWorld);
                ...
                while (chance-- > rnd.NextFloat())
                {
                    ...
                    if (bpc.IsPatchSuitableUnderTree(bPatch, worldheight, climate, y))
                    {
                        ...
                        if (found)
                        {
                            ...
                            // Pass null - no structure intersection check needed here
                            bPatch.Generate(blockAccessor, rnd, tmpPos.X, tmpPos.Y, tmpPos.Z, firstBlockId, null);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                while (chance-- > rnd.NextFloat())
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        if (nblock is IBlockLog && nblock.Variant["type"] != "resin")
                        {
                            facing = f;
                            break;
                        }
        ...
        private void CheckAndReplaceForestFloor(BlockPos pos, int intensity, int grassLevelOffset)
        {
            ...
            if (soilBlock is IBlockForestFloor || soilBlock is IBlockSoil)
            {
                // Update any existing forest floor blocks to which this tree also spreads - intensify forest level
                if (soilBlock is IBlockForestFloor bff)
                {
                    int existingLevel = bff.CurrentLevel();
                    intensity += existingLevel * 18 - 9;
                    intensity = Math.Min(intensity, Math.Max(existingLevel * 18, (ForestFloorHelper.MaxStage - 1) * 18));   // aim not to have it completely brown between trees
                }
